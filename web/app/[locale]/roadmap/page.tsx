...
const tracksEn = [
  {
    ...
    items: [
      ...
      { title: "Durable sessions + tasks", note: "Save, resume, rollback; background task queue with replayable timelines under ~/.codewhale/tasks/ and legacy ~/.deepseek/tasks/" },
      { title: "Bidirectional MCP", note: "Consume tools from external servers; expose as server via `codewhale mcp`; ~/.codewhale/mcp.json with legacy fallback" },
      { title: "Skills + unified slash palette", note: "~/.codewhale/skills/ auto-loading with legacy ~/.deepseek/skills/ fallback; /help, /mode, /status, /config, /trust, /feedback" },
      { title: "OpenRouter provider", note: "First-class OpenRouter integration with 300+ models across dozens of providers" },
      { title: "Multi-provider support", note: "Hot-swap between DeepSeek, OpenRouter, OpenAI-compatible, NVIDIA NIM, Novita, Fireworks, AtlasCloud, Wanjie Ark, Xiaomi MiMo, SGLang, vLLM, and Ollama" },
    ],
  },
  {
    title: "Underway",
...
const tracksZh = [
  {
    ...
    items: [
      ...
      { title: "持久化会话 + 后台任务", note: "保存、恢复、回滚；后台任务队列，可回放时间线，位于 ~/.codewhale/tasks/，并兼容旧 ~/.deepseek/tasks/" },
      { title: "双向 MCP 协议", note: "消费外部服务器工具；通过 `codewhale mcp` 暴露为服务器；~/.codewhale/mcp.json 带旧版回退" },
      { title: "技能 + 统一命令面板", note: "~/.codewhale/skills/ 自动加载，并兼容旧 ~/.deepseek/skills/；/help、/mode、/status、/config、/trust、/feedback" },
      { title: "OpenRouter 提供商", note: "原生集成 OpenRouter，支持 300+ 模型，覆盖数十个提供商" },
      { title: "多提供商支持", note: "按会话动态切换 DeepSeek、OpenRouter、OpenAI 兼容、NVIDIA NIM、Novita、Fireworks、AtlasCloud、万界方舟、小米 MiMo、SGLang、vLLM 和 Ollama" },
    ],
  },
  {
    title: "进行中",
    ...
    items: [
      ...
      { title: "Model Lab provider 打磨", note: "改进 Hugging Face 发现、弃用 provider 迁移和开放模型默认值，同时保持既有配置可用" },
    ],
  },
  {
    title: "考虑中",
