...
# Xiaomi MiMo（Token Plan — cn / sgp / ams クラスター）
codewhale auth set --provider xiaomi --api-key "YOUR_MIMO_API_KEY"
codewhale --provider xiaomi --model mimo-v2.5-pro
# MIMO_CLUSTER=sgp codewhale --provider xiaomi     # シンガポールクラスター
# MIMO_BASE_URL="https://token-plan-ams.xiaomimimo.com/v1" codewhale --provider xiaomi  # アムステルダムクラスター
...
- サブエージェント fanout に panic セーフな completion guard と親側の待機 heartbeat を追加し、子タスク失敗時の turn loop 停止を防ぎます。
- 周期的な context briefing は 120 秒でタイムアウトし、異常時はセッションを無期限に止めずにフォールバックします。
- CJK/IME 入力は composer に直接挿入され、同時に高速な複数文字貼り付け時の Enter 抑制も維持します。
- `/statusline` ピッカーは上下端で循環し、カーソルを表示範囲に保ち、選択行を端末幅いっぱいに強調します。
- Windows 向けリリース成果物に exe と並んで `codewhale.bat` ランチャーが含まれるようになりました。
...
| `CODEWHALE_PROVIDER` / `DEEPSEEK_PROVIDER` | `deepseek`（デフォルト）、`nvidia-nim`、`openai`、`atlascloud`、`wanjie-ark`、`openrouter`、`novita`、`fireworks`、`xiaomi`、`sglang`、`vllm`、`ollama`（`moonshot` / `kimi` は v0.8.48 で非推奨、既存の設定は引き続き読み込み可能） |
...
| `NVIDIA_API_KEY` / `OPENAI_API_KEY` / `ATLASCLOUD_API_KEY` / `WANJIE_ARK_API_KEY` / `OPENROUTER_API_KEY` / `NOVITA_API_KEY` / `FIREWORKS_API_KEY` / `MIMO_API_KEY` / `SGLANG_API_KEY` / `VLLM_API_KEY` / `OLLAMA_API_KEY` | プロバイダー認証 |
...
| `MIMO_BASE_URL` | Xiaomi MiMo エンドポイント上書き |
| `MIMO_CLUSTER` | Xiaomi MiMo リージョンクラスター: `cn`（デフォルト）、`sgp`、`ams` |
...
> [!Note]
> V4 Pro の上記レートは恒久的な料金です。DeepSeek は 2026 年 5 月 31 日に 75% 期間限定割引を恒久的な 4 分の 1 の基本レート調整に組み込みました。TUI のコスト見積もりはすでにこれらの値を使用しています。今後の価格変更については、公式の [DeepSeek 価格ページ](https://api-docs.deepseek.com/zh-cn/quick_start/pricing) を参照してください。
...
## Railway / Telegram リモートエージェント

Telegram から操作できる常駐 worker を米国ホスティングで動かす場合は、まず
リポジトリ直下の Railway 設定を使います。Rust バイナリをビルドし、
`127.0.0.1` で `codewhale serve --http` を起動し、同じ非公開コンテナ内で
Telegram の long polling ブリッジを動かします。詳しくは
[deploy/railway/README.md](deploy/railway/README.md) と
...
| [deploy/railway/README.md](deploy/railway/README.md) | Railway Telegram ワーカー設定 |
| [integrations/telegram-bridge/README.md](integrations/telegram-bridge/README.md) | Telegram スマートフォンブリッジ |
...
v0.8.48 の安定化ハーベストでは、特に **[reidliu41](https://github.com/reidliu41)**、
**[donglovejava](https://github.com/donglovejava)**、**[zlh124](https://github.com/zlh124)**、
**[cyq1017](https://github.com/cyq1017)**、**[cy2311](https://github.com/cy2311)**、
**[nightt5879](https://github.com/nightt5879)**、**[yyyCode](https://github.com/yyyCode)**、
**[buko](https://github.com/buko)**、**[T-Phuong-Nguyen](https://github.com/T-Phuong-Nguyen)** に感謝します。
...
- **[reidliu41](https://github.com/reidliu41)** — statusline ピッカーのナビゲーション、MCP shutdown 防護、provider ヘルプ文言 (#2324, #2357, #2366)
